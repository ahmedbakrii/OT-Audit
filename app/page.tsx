...
export default function Home() {
    ...
    try {
      ...
      if (calculations) {
        calculations.forEach((calc: any) => {
          ...
          const rejected = Number(calc.rejected_hours) || 0; 
  ...
  return (
    <div className="relative w-full min-h-screen">
      ...
      {/* 🔴 استدعاء شاشة المنع الفخمة لمدخل البيانات */}
      {userRole === 'DATA_ENTRY' && (
        <ForbiddenOverlay userDeptId={userDeptId} />
      )}

      {/* 🔴 المحتوى مع تأثير الـ Blur القوي لو المستخدم مدخل بيانات */}
      <div className={`flex flex-col space-y-8 pb-10 transition-all duration-500 ${userRole === 'DATA_ENTRY' ? 'blur-[12px] opacity-30 pointer-events-none select-none grayscale-[50%]' : 'animate-in fade-in'}`}>
